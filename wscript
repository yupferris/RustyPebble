...
TaskGen.declare_chain(
    name='rustc',
    rule='${RUSTC} -L ../lib/ --target ../thumbv7m-none-eabi ${SRC} --emit=llvm-ir -A dead-code -o ${TGT}',
    ext_in=['.rs'],
    ext_out=['.ll'])
...
def build(ctx):
    ...
    for p in ctx.env.TARGET_PLATFORMS:
        ...
        ctx.pbl_program(source=ctx.path.ant_glob('src/*.(rs|c|o)'), target='pebble-app.elf')
